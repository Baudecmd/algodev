...
public class PartiePoker implements Partie {
	...
	public static void main(String[] args) {
		/*
		 * int nbTurns=1; ArrayList<JoueurPoker>winners;
		 * ArrayList<JoueurPoker>finalPlayers; ArrayList<JoueurPoker>quit; //liste
		 * permettant de demander aux joueurs si ils souhaitent quitter la partie
		 * JoueurPoker player1=new JoueurPoker("Joffrey", 500); JoueurPoker player2=new
		 * JoueurPoker("Gaby", 500); Scanner sc=new Scanner(System.in); int choice;
		 * System.out.println("Quelle est la mise de la grosse blinde?"); PartiePoker
		 * partie=new PartiePoker(sc.nextInt()); partie.addPlayer(player1);
		 * partie.addPlayer(player2); while(!partie.partieFinie()){ if(nbTurns==1){
		 * partie.giveCardsToPlayer(); for(JoueurPoker player:PartiePoker.listeJoueurs){
		 * System.out.println(player.getNom() + ", vous avez les cartes suivantes:");
		 * player.showHand(); } partie.nextTurn(true); nbTurns++; } else{
		 * if(nbTurns==5){ System.out.println("C'est le Showdown!"); //A traiter
		 * for(JoueurPoker player:PartiePoker.listeJoueurs){
		 * player.setCombinationHand(player.createAllCombinations(partie.
		 * getCommunityCards())); //on assigne à chaque joueur sa meilleure main
		 * System.out.println(player.getNom() + ", vous avez la main suivante:" );
		 * player.showHand(); System.out.println("Vous avez la combinaison: " +
		 * player.getCombinaison()); System.out.println(); } finalPlayers=new
		 * ArrayList<>(partie.getListeJoueurs()); winners=new ArrayList<>();
		 * Collections.sort(finalPlayers, JoueurPoker::compareCombination); //on trie en
		 * fonction de la meilleure combinaison Collections.reverse(finalPlayers);
		 * winners.add(finalPlayers.get(0)); //c'est forcément un gagnant
		 * for(JoueurPoker player:finalPlayers){
		 * if(player.getCombinaison()==winners.get(0).getCombinaison() &&
		 * !winners.contains(player)) { //la deuxième condition permet d'éviter que le
		 * premier joueur, qui est déjà dans la liste, soit placé deux fois dedans if
		 * (player.bestHand(player.getMainJoueur(), winners.get(0).getMainJoueur())==0)
		 * winners.add(player); else{ if
		 * (player.bestHand(player.getMainJoueur(),winners.get(0).getMainJoueur()) > 0)
		 * { winners.clear(); winners.add(player); } } } } if(winners.size()>1){
		 * System.out.println("Les joueurs:"); for(JoueurPoker player:winners){
		 * System.out.println(player.getNom()); }
		 * System.out.println(" gagnent la manche!"); for(JoueurPoker player:winners)
		 * player.setSomme(player.getSomme()+(partie.getPot()/winners.size())); } else{
		 * System.out.println(winners.get(0).getNom() + " gagne la manche!");
		 * winners.get(0).setSomme(winners.get(0).getSomme()+partie.getPot()); }
		 * nbTurns=1; partie.newRound(); //on réinitialise les variables de classe
		 * quit=new ArrayList<>(partie.getListeJoueurs()); for(JoueurPoker joueur:quit){
		 * joueur.setMainJoueur(new ArrayList<>()); joueur.setTapis(false); //on en
		 * profite pour réinitialiser le tapis des joueurs joueur.setMise(0);
		 * if(joueur.getSomme()==0){ PartiePoker.listeJoueurs.remove(joueur);
		 * System.out.println(joueur.getNom() +
		 * " n'a plus assez d'argent pour continuer! Il est éliminé!");
		 * if(PartiePoker.listeJoueurs.size()==1) break; }
		 * System.out.println(joueur.getNom() +
		 * ", souhaitez-vous partir? Tapez 1 pour dire oui"); choice=sc.nextInt();
		 * if(choice==1) PartiePoker.listeJoueurs.remove(joueur);
		 * if(PartiePoker.listeJoueurs.size()==1) //pas la peine de continuer à
		 * demander s'il ne reste qu'un seul joueur break; } } else { //étapes de flop,
		 * turn ou river partie.addCommunityCards(); partie.showCommunityCards();
		 * partie.nextTurn(false); nbTurns++; } } finalPlayers=new
		 * ArrayList<>(partie.getListeJoueurs());
		 * finalPlayers.removeAll(partie.getJoueurCouches());
		 * if(finalPlayers.size()==1){ if(partie.getJoueurCouches().size()>0){ //tous
		 * les joueurs se sont couchés sauf un
		 * System.out.println("Tous les joueurs se sont couchés sauf " +
		 * finalPlayers.get(0).getNom());
		 ...
		 * nbTurns=1; partie.newRound(); quit=new ArrayList<>(partie.getListeJoueurs());
		 * for(JoueurPoker joueur:quit){ if(joueur.getSomme()==0){
		 * PartiePoker.listeJoueurs.remove(joueur); System.out.println(joueur.getNom() +
		 * " n'a plus assez d'argent pour continuer! Il est éliminé!");
		 * if(PartiePoker.listeJoueurs.size()==1) break; } joueur.setTapis(false);
		 * joueur.setMise(0); joueur.setMainJoueur(new ArrayList<>());
		 * System.out.println(joueur.getNom() +
		 * ", souhaitez-vous partir? Tapez 1 pour dire oui"); choice=sc.nextInt();
		 * if(choice==1) PartiePoker.listeJoueurs.remove(joueur);
		 * if(PartiePoker.listeJoueurs.size()==1) //pas la peine de continuer à
		 * demander s'il ne reste qu'un seul joueur break; } } else //il ne reste qu'un
		 * joueur en lice if(finalPlayers.size()==1) break; } } System.out.
		 * println("La partie est terminée! Trop de joueurs ont quitté la partie");
		 */
	}
